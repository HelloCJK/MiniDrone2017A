...
while /bin/true; do
  if [ -z "$(ps ax | grep '[D]roneTest')" ] && [ -z "$(ps ax | grep '[D]roneRun')" ] && [ -n "$(ps ax | grep '[D]roneKeyboardPilot')" ]; then
	/usr/bin/expect -c 'set timeout -1; spawn telnet 192.168.2.1; expect "RS.edu] \$ "; send "killall -s SIGKILL dragon-prog; gpio 39 -d ho 1; test-SIP6_pwm -S\r"; expect "RS.edu] \$ ";send "exit\r"; expect eof'
