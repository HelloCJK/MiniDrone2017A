...
char filename[20]="/data/edu/image.uyvy";
...
int y;
  	 ...
  	 FILE* data;
		...
		//read image data
		printf("\n\n@@@@@@@@@@@@@@@@@@@@@@@@ Y1 : %d, U : %d, Y2 : %d, V : %d\n @@@@@@@@@@@@@@@@@@@@@@ \n\n",
				(int)fileimage[nx/2].y1,
				(int)fileimage[nx/2].u,
				(int)fileimage[nx/2].y2,
				(int)fileimage[nx/2].v);
		/*for (j=0; j<ny; j++)
		{
		 for (i=0; i<nx; i++)
		   {
...
int onCycles 		= 12000; //20sec total
int calibCycles 	= 400;
...
void RSEDU_control(HAL_acquisition_t* hal_sensors_data, HAL_command_t* hal_sensors_cmd)
{
    ...
    else if(counter <= onCycles)
    {
        ...
        if(!(FEAT_TEST)&&( ((!FEAT_NOSAFETY) && ((abs(in->HAL_acc_SI.x) > MAX_ACCELL) || (abs(in->HAL_acc_SI.y) > MAX_ACCELL) || (in->HAL_acc_SI.z > 0)))
                ||
